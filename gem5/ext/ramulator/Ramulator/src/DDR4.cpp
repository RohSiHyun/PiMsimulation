...
void DDR4::init_prereq()
{
	...
	prereq[int(Level::Rank)][int(Command::PIM)] = prereq[int(Level::Rank)][int(Command::RD)];
	prereq[int(Level::Bank)][int(Command::PIM)] = prereq[int(Level::Bank)][int(Command::RD)];
...
void DDR4::init_rowhit()
{
	...
	rowhit[int(Level::Bank)][int(Command::PIM)] = rowhit[int(Level::Bank)][int(Command::RD)];
}
...
void DDR4::init_rowopen()
{
	...
	rowopen[int(Level::Bank)][int(Command::PIM)] = rowopen[int(Level::Bank)][int(Command::RD)];
}
...
void DDR4::init_lambda()
{
    ...
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<DDR4>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
	lambda[int(Level::Bank)][int(Command::PIM)] = [] (DRAM<DDR4>* node, int id) {};
	lambda[int(Level::Bank)][int(Command::PIMA)] = [] (DRAM<DDR4>* node, int id){
		node->state = State::Closed;
		node->row_state.clear();};
...
void DDR4::init_timing()
{
	...
	t[int(Command::PIM)].push_back({Command::PIM, 1, s.nBL});
	t[int(Command::PIM)].push_back({Command::PIMA, 1, s.nBL});
	t[int(Command::PIMA)].push_back({Command::PIM, 1, s.nBL});
	t[int(Command::PIMA)].push_back({Command::PIMA, 1, s.nBL});
	...
	t[int(Command::PIM)].push_back({Command::PIM, 1, s.nCCDS});
	t[int(Command::PIM)].push_back({Command::PIMA, 1, s.nCCDS});
	t[int(Command::PIMA)].push_back({Command::PIM, 1, s.nCCDS});
	t[int(Command::PIMA)].push_back({Command::PIMA, 1, s.nCCDS});
	...
	t[int(Command::RD)].push_back({Command::PIM, 1, s.nCL + s.nBL + 2 - s.nCWL});
	t[int(Command::RD)].push_back({Command::PIMA, 1, s.nCL + s.nBL + 2 - s.nCWL});
	...
	t[int(Command::RDA)].push_back({Command::PIM, 1, s.nCL + s.nBL + 2 - s.nCWL});
	t[int(Command::RDA)].push_back({Command::PIMA, 1, s.nCL + s.nBL + 2 - s.nCWL});
	...
	t[int(Command::WR)].push_back({Command::PIM, 1, s.nCCDS});
	t[int(Command::WR)].push_back({Command::PIMA, 1, s.nCCDS});
	...
	t[int(Command::WRA)].push_back({Command::PIM, 1, s.nCCDS});
	t[int(Command::WRA)].push_back({Command::PIMA, 1, s.nCCDS});
	t[int(Command::PIM)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRS});
	t[int(Command::PIM)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRS});
	t[int(Command::PIM)].push_back({Command::WR, 1, s.nCCDS});
	t[int(Command::PIM)].push_back({Command::WRA, 1, s.nCCDS});
	t[int(Command::PIMA)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRS});
	t[int(Command::PIMA)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRS});
	t[int(Command::PIMA)].push_back({Command::WR, 1, s.nCCDS});
	t[int(Command::PIMA)].push_back({Command::WRA, 1, s.nCCDS});
	...
	t[int(Command::RD)].push_back({Command::PIM, 1, s.nBL + s.nRTRS, true});
	t[int(Command::RD)].push_back({Command::PIMA, 1, s.nBL + s.nRTRS, true});
	t[int(Command::RDA)].push_back({Command::PIM, 1, s.nBL + s.nRTRS, true});
	t[int(Command::RDA)].push_back({Command::PIMA, 1, s.nBL + s.nRTRS, true});
	...
	t[int(Command::RD)].push_back({Command::PIM, 1, s.nCL + s.nBL + s.nRTRS - s.nCWL, true});
	t[int(Command::RD)].push_back({Command::PIMA, 1, s.nCL + s.nBL + s.nRTRS - s.nCWL, true});
	...
	t[int(Command::RDA)].push_back({Command::PIM, 1, s.nCL + s.nBL + s.nRTRS - s.nCWL, true});
	t[int(Command::RDA)].push_back({Command::PIMA, 1, s.nCL + s.nBL + s.nRTRS - s.nCWL, true});
	...
	t[int(Command::PIM)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nRTRS - s.nCL, true});
	t[int(Command::PIM)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nRTRS - s.nCL, true});
	t[int(Command::PIMA)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nRTRS - s.nCL, true});
	t[int(Command::PIMA)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nRTRS - s.nCL, true});

    t[int(Command::RD)].push_back({Command::PREA, 1, s.nRTP});
    t[int(Command::WR)].push_back({Command::PREA, 1, s.nCWL + s.nBL + s.nWR});
	t[int(Command::PIM)].push_back({Command::PREA, 1, s.nCWL + s.nBL + s.nWR + nPIM});
    // CAS <-> PD
    ...
    t[int(Command::WRA)].push_back({Command::PDE, 1, s.nCWL + s.nBL + s.nWR + 1}); 
	t[int(Command::PIM)].push_back({Command::PDE, 1, s.nCWL + s.nBL + s.nWR + nPIM});
	t[int(Command::PIMA)].push_back({Command::PDE, 1, s.nCWL +s.nBL + s.nWR +nPIM+ 1});
	...
	t[int(Command::PDX)].push_back({Command::PIM, 1, s.nXP});
	t[int(Command::PDX)].push_back({Command::PIMA, 1, s.nXP});
	...
	t[int(Command::PIMA)].push_back({Command::REF, 1, s.nCWL + s.nBL + s.nWR + s.nRP});
	...
	t[int(Command::PIM)].push_back({Command::PIM, 1, s.nCCDL});
	t[int(Command::PIM)].push_back({Command::PIMA, 1, s.nCCDL});
	t[int(Command::PIMA)].push_back({Command::PIM, 1, s.nCCDL});
	t[int(Command::PIMA)].push_back({Command::PIMA, 1, s.nCCDL});
	...
	t[int(Command::WR)].push_back({Command::PIM, 1, s.nCCDL});
	t[int(Command::WR)].push_back({Command::PIMA, 1, s.nCCDL});
	...
	t[int(Command::WRA)].push_back({Command::PIM, 1, s.nCCDL});
	t[int(Command::WRA)].push_back({Command::PIMA, 1, s.nCCDL});
	t[int(Command::PIM)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRL + nPIM});
	t[int(Command::PIM)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRL + nPIM});
	t[int(Command::PIM)].push_back({Command::WR, 1, s.nCCDL + nPIM});
	t[int(Command::PIM)].push_back({Command::WRA, 1, s.nCCDL + nPIM});
	t[int(Command::PIMA)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRL + nPIM});
	t[int(Command::PIMA)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRL + nPIM});
	t[int(Command::PIMA)].push_back({Command::WR, 1, s.nCCDL+ nPIM});
	t[int(Command::PIMA)].push_back({Command::WRA, 1, s.nCCDL + nPIM});
	...
	t[int(Command::ACT)].push_back({Command::PIM, 1, s.nRCD});
	t[int(Command::ACT)].push_back({Command::PIMA, 1, s.nRCD});

    t[int(Command::RD)].push_back({Command::PRE, 1, s.nRTP});
    t[int(Command::WR)].push_back({Command::PRE, 1, s.nCWL + s.nBL + s.nWR});
	t[int(Command::PIM)].push_back({Command::PRE, 1, s.nCWL + s.nBL + s.nWR + nPIM});
    t[int(Command::RDA)].push_back({Command::ACT, 1, s.nRTP + s.nRP});
    t[int(Command::WRA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR + s.nRP});
	t[int(Command::PIMA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR + s.nRP + nPIM});
    // RAS <-> RAS

...
protected:
	...
	ScalarStat pim_transaction_bytes;
	...
	VectorStat pim_row_hits;
	VectorStat pim_row_misses;
	VectorStat pim_row_conflicts;
	...
	ScalarStat pim_req_queue_length_avg;
	ScalarStat pim_req_queue_length_sum;
	...
	VectorStat record_pim_hits;
	VectorStat record_pim_misses;
	VectorStat record_pim_conflicts;
#endif
...
public:
	...
	bool pim_mode = false;
	float pm_high_watermark = 0.8f;
	float pm_low_watermark = 0.2f;
        ...
        cmd_trace_files(channel->children.size())
    {
		...
		pim_row_hits
			.init(configs.get_core_num())
			.name("pim_row_hits_channel_"+to_string(channel->id) + "_core")
			.desc("Number of row hits for pim requests per channel per core")
			.precision(0)
			;
		pim_row_misses
			.init(configs.get_core_num())
			.name("pim_row_misses_channel_" + to_string(channel->id) + "_core")
			.desc("Number of row misses for pim requests per channel per core")
			.precision(0)
			;
		pim_row_conflicts
			.init(configs.get_core_num())
			.name("pim_row_conflicts_channel_"+to_string(channel->id)+"_core")
			.desc("Number of row conflicts for pim requests per channel per core")
			.precision(0)
			;
        useless_activates
        ...
        write_transaction_bytes
            ...
            .precision(0)
            ;
		pim_transaction_bytes
			.name("pim_transaction_bytes_"+to_string(channel->id))
			.desc("The total byte of pim transaction per channel")
			.precision(0)
			;
        read_latency_sum
        ...
        write_req_queue_length_avg
            ...
            .precision(6)
            ;
		pim_req_queue_length_sum
			.name("pim_req_queue_length_sum_"+to_string(channel->id))
			.desc("Pim queue length sum per memory cycle per channel.")
			.precision(0)
			;
		pim_req_queue_length_avg
			.name("pim_req_queue_length_avg_"+to_string(channel->id))
			.desc("Pim queue length average per memory cycle per channel.")
			.precision(6)
			;
#ifndef INTEGRATED_WITH_GEM5
        record_read_hits
		...
		record_pim_hits
			.init(configs.get_core_num())
			.name("record_pim_hits")
			.desc("record pim hit count for this core when it reaches request limit or to the end")
			;
		record_pim_misses
			.init(configs.get_core_num())
			.name("record_pim_misses")
			.desc("record pim miss count for this core when it reaches request limit or to the end")
			;
		record_pim_conflicts
			.init(configs.get_core_num())
			.name("record_pim_conflicts")
			.desc("record pim conflicts for this core when it reaches request limit or to the end")
			;
#endif
    }
    ...
    void finish(long read_req, long dram_cycles) {
	  ...
	  pim_req_queue_length_avg = pim_req_queue_length_sum.value() / dram_cycles;
    ...
    Queue& get_queue(Request::Type type)
    {
        switch (int(type)) {
			...
			case int(Request::Type::PIM): return pimq;
    ...
    bool enqueue(Request& req)
    {
        ...
        if (req.type == Request::Type::READ && (find_if(writeq.q.begin(), writeq.q.end(),
                [req](Request& wreq){ return req.addr == wreq.addr;}) != writeq.q.end() 
					||  find_if(pimq.q.begin(), pimq.q.end(), [req](Request& preq){return req.addr == preq.addr;}) != pimq.q.end()) ){
            req.depart = clk + 1;
    ...
    void tick()
    {
		...
		pim_req_queue_length_sum += pimq.size();
        ...
        if (!is_valid_req) {
            queue = !pim_mode ? (!write_mode ? &readq : &writeq): &pimq;
        ...
        if (req->is_first_command) {
            ...
            if (req->type == Request::Type::READ || req->type == Request::Type::WRITE || req->type == Request::Type::PIM) {
            ...
            } else if (req->type == Request::Type::PIM){
			  if (is_row_hit(req)) {
				  ++pim_row_hits[coreid];
				  ++row_hits;
			  } else if(is_row_open(req)) {
				  ++pim_row_conflicts[coreid];
				  ++row_conflicts;
			  } else {
				  ++pim_row_misses[coreid];
				  ++row_misses;
			  }
			  pim_transaction_bytes += tx;
			}
        }
	...
	void set_high_pimq_watermark(const float watermark){
		pm_high_watermark = watermark;
	}
	void set_low_pimq_watermark(const float watermark){
		pm_low_watermark = watermark;
	}
    ...
    void record_core(int coreid) {
	  ...
	  record_pim_hits[coreid] = pim_row_hits[coreid];
	  record_pim_misses[coreid] = pim_row_misses[coreid];
	  record_pim_conflicts[coreid] = pim_row_conflicts[coreid];
#endif
    }
...
private:
    ...
    void cmd_issue_autoprecharge(typename T::Command& cmd,
        ...
        if(channel->spec->is_accessing(cmd) && rowpolicy->type == RowPolicy<T>::Type::ClosedAP) {
            ...
            Queue* queue = pim_mode ? &pimq : (write_mode ? &writeq : &readq);

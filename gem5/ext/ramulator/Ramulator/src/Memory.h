...
public:
	...
	virtual void set_high_pimq_watermark(const float watermark) = 0;
	virtual void set_low_pimq_watermark(const float watermark) = 0;
};
...
public:
          ...
          addr_bits(int(T::Level::MAX))
    {
		...
		num_pim_requests
			.init(configs.get_core_num())
			.name("pim_requests")
			.desc("Number of incoming pim requests to DRAM per core")
			.precision(0)
			;
        ...
        in_queue_write_req_num_sum
            ...
            .precision(0)
            ;
		in_queue_pim_req_num_sum
			.name("in_queue_pim_req_num_sum")
			.desc("Sum of pim queue length")
			.precision(0)
			;
        ...
        in_queue_write_req_num_avg
            ...
            .precision(6)
            ;
		in_queue_pim_req_num_avg
			.name("in_queue_pim_req_num_avg")
			.desc("Average of pim queue length per memory cycle")
			.precision(6)
			;
#ifndef INTEGRATED_WITH_GEM5
		...
		record_pim_requests
			.init(configs.get_core_num())
			.name("record_pim_requests")
			.desc("record pim requests for this core when it reaches request limit or to the end")
			;
#endif
    ...
    void record_core(int coreid) {
	  ...
	  record_pim_requests[coreid] = num_pim_requests[coreid];
#endif
      for (auto ctrl : ctrls) {
    ...
    void tick()
    {
        ...
        for (auto ctrl : ctrls) {
		  ...
		  cur_que_pimreq_num += ctrl->pimq.size();
        }
		...
		in_queue_pim_req_num_sum += cur_que_pimreq_num;
	...
	void set_high_pimq_watermark(const float watermark) {
		for (auto ctrl: ctrls)
			ctrl->set_high_pimq_watermark(watermark);
	}

	void set_low_pimq_watermark(const float watermark){
		for(auto ctrl: ctrls)
			ctrl->set_low_pimq_watermark(watermark);
    ...
    void finish(void) {
	  ...
	  in_queue_pim_req_num_avg = in_queue_pim_req_num_sum.value() / dram_cycles;
    }
